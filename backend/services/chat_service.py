...
RESPONSE: <if CHAT, write a friendly response>"""
    ...
    def _fallback_parse(self, message):
        ...
        if ('email' in message_lower or 'mail' in message_lower or 'send' in message_lower) and email_match:
            intent = 'email'
    ...
    def execute_query(self, parsed_data, user_id):
        ...
        # Use context photos if requested
    ...
    def _handle_context_delivery(self, parsed_data, user_id, context):
        ...
        return {
            ...
            'count': len(photos),
            'message': f"Ready to email {len(photos)} photos to {parsed_data['recipient']}"
        }
    ...
    def _prepare_email_delivery(self, parsed_data, user_id):
        ...
        return {
            ...
            'recipient': parsed_data['recipient'],
            'count': search_result['count'],
            'message': f"Ready to email {search_result['count']} photos to {parsed_data['recipient']}"
        }
